...
export const updateAppointment = async ({
  ...
  try {
    ...
    const emailSubject = type === "schedule" ? "Appointment Confirmed" : "Appointment Cancelled";
    ...
    // Send the email using the patient's email stored in the document
    if (updatedAppointment.patientId?.email) {
      await sendEmailNotification(updatedAppointment.patientId.email, emailBody);
    }
