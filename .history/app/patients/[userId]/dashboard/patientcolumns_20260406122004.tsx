...
// Rename this to patientColumns (plural/camelCase) so it's clearly data, not a component
export const patientColumns: ColumnDef<Appointment>[] = [
  ...
  },
  {
    accessorKey: "reason",
    header: "Notes",
    cell: ({ row }) => <p className="text-14-medium min-w-[150px]">{row.original.reason}</p>,
  },
  {
    ...
    cell: ({ row }) => (
      <p className="text-14-regular text-blue-500">
        {row.original.status === "cancelled" ? row.original.cancellationReason : "N/A"}
      </p>
    ),
  },
];